view: third_max_month {
  derived_table: {
    sql: SELECT
      ...
      CAST(FORMAT_TIMESTAMP('%m', CAST(sku_data.DATE  AS TIMESTAMP))  AS NUMERIC) = (SELECT MAX(CAST(FORMAT_TIMESTAMP('%m', CAST(sku_data.DATE  AS TIMESTAMP)) AS NUMERIC)) - 2 FROM  demand_forecast.sku_data )
